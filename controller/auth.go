...
func CreateTokenHandler(c *gin.Context) {
	...
	// 设置默认插件类型
	if tokenData.PluginType == "" {
		tokenData.PluginType = "all"
	}
	...
	// 检查插件类型
	if tokenData.PluginType != "postgresql" && tokenData.PluginType != "mongodb" && tokenData.PluginType != "all" {
		utils.ErrorResponse(c, 1400, "插件类型必须是 postgresql, mongodb 或 all")
		return
	}
...
func UpdateTokenHandler(c *gin.Context) {
	...
	if !success {
		...
		if err != nil {
			...
			if errorMessage == fmt.Sprintf("未找到令牌 %s", updateData.AccessToken) {
				errorCode = 1051
...
func GetTokenInfoHandler(c *gin.Context) {
	wsID := c.Query("ws_id")
	pluginTypeParam := c.Query("plugin_type")
	...
	var pluginTypePtr *string
	if pluginTypeParam != "" {
		// 验证plugin_type参数
		if pluginTypeParam != "postgresql" && pluginTypeParam != "mongodb" && pluginTypeParam != "all" {
			utils.ErrorResponse(c, 1400, "插件类型必须是 postgresql, mongodb 或 all")
			return
		}
		pluginTypePtr = &pluginTypeParam
	}

	// 查询令牌信息
	success, tokenInfo, err := database.GetAuthTokenInfo(wsID, pluginTypePtr)

	if !success {
		var errorCode int
		if err != nil {
			errorMessage := err.Error()
			if strings.Contains(errorMessage, "未找到工作区ID") {
				errorCode = 1051
