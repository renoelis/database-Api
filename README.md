...
- **自动日志清理机制** - 每天定时清理旧的令牌使用日志，保持数据库高效
...
为了优化数据库性能并避免日志表无限增长，系统实现了自动日志清理机制：

#### 自动日志清理

- **定时执行**：系统在每天凌晨3点自动清理过期日志
- **保留策略**：只保留最近1个月的日志记录，删除更早的记录
- **故障恢复**：清理任务异常中断后会在1小时后重试
- **启动自动化**：服务启动时自动初始化定时清理任务

#### 手动清理接口

系统也提供了手动触发清理的API接口，用于管理和测试：

```
POST /apiDatabase/auth/logs/cleanup
```
...
- 每次调用PostgreSQL或MongoDB的写操作（POST, PUT, PATCH, DELETE请求）会自动扣减有限制令牌的使用次数
- 无限制类型的令牌不会扣减次数，但同样会记录使用日志
- 查询操作（GET请求）不消耗令牌次数也不记录使用日志
- 当`remaining_calls`降至0时，API将拒绝后续请求，直到增购令牌次数
- 系统会自动记录每次令牌使用的详细信息，并保留最近1个月的记录
